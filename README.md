...
- **Propuestas** (`/propuestas`): todas las cotizaciones (las propias; un admin ve todas), con filtro Todas/En revisión/Generadas y búsqueda por cliente o referencia. Generar no cierra nada: cada fila tiene *Seguir editando* y *Generar otra versión* siempre disponibles, y se expande para mostrar el historial de PDF generados de esa cotización, numerados por versión y con la más reciente marcada —cada propuesta base y cada presentación editorial, con su fecha— con "Abrir para imprimir" (visor del navegador) y "Descargar" (fuerza guardar el archivo). Si no hay ninguno, enlaza de vuelta a Revisar.
- **Revisar** (`/cotizaciones/:id`): tabla con pendientes arriba; "Elegir imagen" abre *Biblioteca* (o *Subir foto* en ítems fuera de catálogo) y *Generar imagen con IA*. Al elegir una de las 4 opciones generadas, las otras 3 se borran. Si se alcanza el tope diario, el aviso ámbar dice cuál límite y cuándo se libera.
  ...
  - Tarjeta *Moneda e idioma*: elegir pesos o dólares (con el tipo de cambio) e inglés o español sin salir de aquí. Los precios de la tabla y los totales ya salen convertidos, y debajo de cada descripción aparece su traducción, para revisarlo todo antes de imprimir.
...
| PUT | `/cotizaciones/{id}/presentacion/formato` | `{moneda, tipo_cambio, idioma}` desde Revisar, sin tocar el resto |
...
- **La moneda y el idioma se editan desde Revisar y desde el editor**, con un endpoint propio (`/presentacion/formato`) que sólo toca esos tres campos: así guardar desde una pantalla no pisa lo que se escribió en la otra.
- **Generar un PDF no bloquea la cotización**: se sigue editando y cada generación queda como una versión más en Propuestas. Volver a subir el mismo PDF del sistema crearía una cotización duplicada, que es justo lo que se evita.
